...
## CUDA 10.1 updated
# from https://hub.docker.com/layers/nvidia/cuda/10.1-cudnn7-devel-ubuntu18.04/images/sha256-52596ec3e9061fbad554a4f5eefcf72fef3568d1c085de4eb8436cf5a98477d8?context=explore
# RUN apt-get update && apt-get install -y --no-install-recommends     gnupg2 curl ca-certificates &&     curl -fsSL https://developer.download.nvidia.com/compute/cuda/repos/ubuntu1804/x86_64/7fa2af80.pub | apt-key add - &&     echo "deb https://developer.download.nvidia.com/compute/cuda/repos/ubuntu1804/x86_64 /" > /etc/apt/sources.list.d/cuda.list &&     echo "deb https://developer.download.nvidia.com/compute/machine-learning/repos/ubuntu1804/x86_64 /" > /etc/apt/sources.list.d/nvidia-ml.list &&     apt-get purge --autoremove -y curl     && rm -rf /var/lib/apt/lists/* # buildkit
# ENV CUDA_VERSION=10.1.243
# ENV CUDA_PKG_VERSION=10-1=10.1.243-1
# RUN apt-get update && apt-get install -y --no-install-recommends     cuda-cudart-$CUDA_PKG_VERSION     cuda-compat-10-1     && ln -s cuda-10.1 /usr/local/cuda &&     rm -rf /var/lib/apt/lists/* # buildkit
...
# ENV NCCL_VERSION=2.7.8
# RUN apt-get update && apt-get install -y --no-install-recommends     cuda-libraries-$CUDA_PKG_VERSION     cuda-npp-$CUDA_PKG_VERSION     cuda-nvtx-$CUDA_PKG_VERSION     libcublas10=10.2.1.243-1     libnccl2=$NCCL_VERSION-1+cuda10.1     && apt-mark hold libnccl2     && rm -rf /var/lib/apt/lists/* # buildkit
# RUN apt-mark hold libcublas10 # buildkit
# ENV NCCL_VERSION=2.7.8
# RUN apt-get update && apt-get install -y --no-install-recommends     cuda-nvml-dev-$CUDA_PKG_VERSION     cuda-command-line-tools-$CUDA_PKG_VERSION     cuda-nvprof-$CUDA_PKG_VERSION     cuda-npp-dev-$CUDA_PKG_VERSION     cuda-libraries-dev-$CUDA_PKG_VERSION     cuda-minimal-build-$CUDA_PKG_VERSION     libcublas-dev=10.2.1.243-1     libnccl-dev=2.7.8-1+cuda10.1     && apt-mark hold libnccl-dev     && rm -rf /var/lib/apt/lists/* # buildkit
# RUN apt-mark hold libcublas-dev # buildkit
# ENV LIBRARY_PATH=/usr/local/cuda/lib64/stubs
# ENV CUDNN_VERSION=7.6.5.32
# RUN apt-get update && apt-get install -y --no-install-recommends     libcudnn7=$CUDNN_VERSION-1+cuda10.1     libcudnn7-dev=$CUDNN_VERSION-1+cuda10.1     && apt-mark hold libcudnn7 &&     rm -rf /var/lib/apt/lists/* # buildkit
